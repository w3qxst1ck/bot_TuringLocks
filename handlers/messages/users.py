...
def instruction_os_message(os: str) -> str:
    ...
    if os == "iOS":
        ...
        message += "\t• <a href='https://github.com/hiddify/hiddify-next/releases/download/v2.0.5/Hiddify-Windows-Setup-x64.exe'>Hiddify</a>\n" \
                   "\t• <a href='https://github.com/LorenEteval/Furious'>Furious</a>\n" \
    ...
    message += "‼️Выключайте VРN перед тем, как открыть приложения банков и электронных кошельков\n\n"

    message += f"Для возврата к перечню инструкций по установке vpn нажмите /{cmd.INSTRUCTION[0]}\n\n"
...
def help_message() -> str:
    """Help сообщение"""
    message = f"Для доступа к VPN, вам необходимо купить VPN ключ 🔑 в разделе /{cmd.BUY[0]}, новым пользователям выдается пробный ключ (сроком на {settings.trial_days} дней), он будет отображен в ваших ключах в разделе /{cmd.KEYS[0]} в период его действия\n\n" \
              f"<b>💰Баланс</b>\nПополнить баланс вы можете телеграм звездами в разделе /{cmd.BALANCE[0]}\n\n" \
    ...
    message += f"\n\n❕Ознакомиться с Пользовательским соглашением (Публичной офертой) вы можете по ссылке: https://telegra.ph/Polzovatelskoe-soglashenie-Publichnaya-oferta-06-26"
